...
async def generate_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await monitor_otp_telegram(context, waiting_msg, order_id, number, user.id)

async def monitor_otp_telegram(context, message, order_id, number, user_id):
            ...
            if result and result.get("success"):
                ...
                if status == 1:  # OTP received
                    ...
                    await message.edit_text(
                        ...
                        f"Credits will be refunded automatically.",
                        parse_mode='HTML'
                    )
                    return
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "generate":
        await query.message.reply_text("Please use /generate command to purchase a number.")
